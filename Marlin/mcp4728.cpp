...
uint8_t mcp4728::writeGain() {
  Wire.beginTransmission(_dev_address);
  Wire.write(GAINWRITE | _gain[0] << 3 | _gain[1] << 2 | _gain[2] << 1 | _gain[3]);
  return Wire.endTransmission();
}
/*
...
uint8_t mcp4728::writePowerDown() {
  Wire.beginTransmission(_dev_address);
  Wire.write(POWERDOWNWRITE | _powerDown[0] << 2 | _powerDown[1]);
  Wire.write(_powerDown[2] << 6 | _powerDown[3] << 4);
  return Wire.endTransmission();
}
/*
